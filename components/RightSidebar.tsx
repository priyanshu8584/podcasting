...
const RightSidebar = () => {
  ...
  return (
    <div className='right_sidebar text-white-1'>
      <SignedIn>
        <Link href={`/profile/${user?.user?.id}`} className='flex gap-3 pb-12'>
        <UserButton/>
        <div className='flex w-full items-center justify-between'>
        <h1 className="text-16 truncate font-semibold text-white-1">{user?.user?.firstName} {user?.user?.lastName}</h1>
